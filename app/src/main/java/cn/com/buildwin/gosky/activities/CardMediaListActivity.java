...
//        mSegmentedGroup.check(R.id.card_media_device_files_radioButton);  // Bug, called two times
    ...
    private void loadRemoteVideoList() {
        ...
        // 显示HUD
        mHud = createIndeterminateProgressHUD("Fetching video list...", null).show();
    ...
    private void downloadFiles(List<RemoteFile> fileList) {
        ...
        // 显示HUD
        mHud = createDeterminateProgressHUD(null, "Preparing to download " + fileList.size() + " files", 0);
        ...
        mHud.setOnCancelListener(new DialogInterface.OnCancelListener() {
            ...
            public void onCancel(DialogInterface dialog) {
                dismissProgressHUD();
                mHud = createIndeterminateProgressHUD(null, "Cancelling download...").show();
        ...
        MediaManager.getInstance().downloadRemoteFiles(fileList, new MediaDownloadListener() {
            ...
            public void started(final String fileName, final long total) {
                runOnUiThread(new Runnable() {
                    @Override
                    public void run() {
                        mHud.setLabel(fileName);
                        mHud.setMaxProgress(total);
                        mHud.setProgress(0);
                        mHud.show();
                    }
                });
            ...
            public void transferred(final long length, final long total) {
                runOnUiThread(new Runnable() {
                    @Override
                    public void run() {
                        mHud.setProgress((int)length);
                        mHud.setDetailsLabel("Downloaded: " + Utilities.memoryFormatter(length) + "/" + Utilities.memoryFormatter(total));
                    }
                });
            }
...
//    }
    ...
    private void doStartRemoteRecord() {
        ...
        mHud = createIndeterminateProgressHUD("Exiting remote file manager...", null).show();
